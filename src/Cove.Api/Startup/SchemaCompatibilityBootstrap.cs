...
internal static class SchemaCompatibilityBootstrap
{
    ...
    public static async Task EnsureCompatibilitySchemaAsync(CoveContext db)
    {
        ...
        await EnsureTableExists("field_provenance", """
        ...
        var fieldProvenanceIndexCommands = new[]
        {
            new { Relation = "field_provenance", Sql = "CREATE INDEX IF NOT EXISTS \"IX_field_provenance_HostType_HostId\" ON \"field_provenance\" (\"HostType\", \"HostId\")" },
            new { Relation = "field_provenance", Sql = "CREATE INDEX IF NOT EXISTS \"IX_field_provenance_HostType_HostId_FieldKey\" ON \"field_provenance\" (\"HostType\", \"HostId\", \"FieldKey\")" },
            new { Relation = "field_provenance", Sql = "CREATE INDEX IF NOT EXISTS \"IX_field_provenance_SourceKey\" ON \"field_provenance\" (\"SourceKey\")" },
            new { Relation = "field_provenance", Sql = "CREATE UNIQUE INDEX IF NOT EXISTS \"IX_field_provenance_HostType_HostId_FieldKey_SourceKey_SourceR~\" ON \"field_provenance\" (\"HostType\", \"HostId\", \"FieldKey\", \"SourceKey\", \"SourceRunId\", \"ModelKey\")" },
        };

        foreach (var indexCommand in fieldProvenanceIndexCommands)
        {
            var relationKind = await GetRelationKindAsync(indexCommand.Relation);
            if (relationKind is not ("r" or "p"))
                continue;
