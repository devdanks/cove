...
public sealed class PostgresPerformanceFixture : IAsyncLifetime
{
    ...
    private static async Task EnsureCompatibilityColumnsAsync(CoveContext db)
    {
        ...
        async Task AddColumnIfMissing(string table, string column, string type, bool skipIfTableMissing = false)
        {
            await using var relationCommand = connection.CreateCommand();
            relationCommand.CommandText = $"SELECT 1 FROM information_schema.tables WHERE table_name='{table}'";
            var relationExists = await relationCommand.ExecuteScalarAsync();
            if (relationExists == null)
            {
                if (skipIfTableMissing)
                {
                    return;
                }

                throw new InvalidOperationException($"Compatibility bootstrap expected table '{table}' to exist.");
            }
        ...
        await AddColumnIfMissing("VideoCaptions", "Id", "integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY", skipIfTableMissing: true);
        await AddColumnIfMissing("segment_display_rules", "ProfileId", "integer", skipIfTableMissing: true);
        await AddColumnIfMissing("group_items", "SourceQueryJson", "text", skipIfTableMissing: true);
