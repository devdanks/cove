...
describe("createManualOpenRequest", () => {
  ...
  it("orders the most specific settings context before parent settings contexts", () => {
    expect(createManualOpenRequest("settings", "settings", "/settings/extensions/docs/topic").contexts).toEqual(expect.arrayContaining([
      "settings-tab:extensions/docs/topic",
      "settings-tab:extensions/docs",
    ]));

    const contexts = createManualOpenRequest("settings", "settings", "/settings/extensions/docs/topic").contexts ?? [];
    expect(contexts.indexOf("settings-tab:extensions/docs/topic")).toBeLessThan(contexts.indexOf("settings-tab:extensions/docs"));
  });
