...
import { entityImages, groups as groupsApi, performers as performersApi, tags as tagsApi, texts } from "../api/client";
...
export function TextEditPanel({ text, onSaved }: Props) {
  ...
  const selectedTags = selectedTagIds
    .map((tagId) => allTags?.items.find((tag) => tag.id === tagId) ?? text.tags.find((tag) => tag.id === tagId))
    .filter((tag): tag is NonNullable<typeof tag> => Boolean(tag));
  const selectedPerformers = selectedPerformerIds
    .map((performerId) => allPerformers?.items.find((performer) => performer.id === performerId) ?? text.performers.find((performer) => performer.id === performerId))
  ...
  const selectedGroupItems = selectedGroups
    .map((groupInput) => allGroups?.items.find((group) => group.id === groupInput.groupId) ?? text.groups.find((group) => group.id === groupInput.groupId))
  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-4 lg:grid-cols-[220px_minmax(0,1fr)]">
        <ImageInput
          currentImageUrl={text.imagePath ?? undefined}
