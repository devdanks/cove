...
import { scenes, system } from "../api/client";
import type { DownloaderMatch, Scene } from "../api/types";
import { sortDownloaderMatches } from "./sceneScrapeUtils";

interface Props {
  open: boolean;
  onClose: () => void;
  onNavigate: (route: any) => void;
  scene?: Pick<Scene, "id" | "title" | "urls" | "files">;
}

function deriveSceneTitle(url: string, fallback?: string) {
  if (fallback?.trim()) {
...
export function SceneDownloadDialog({ open, onClose, onNavigate, scene }: Props) {
  ...
  useEffect(() => {
    ...
    setQualityId("");
    setAutoApplyMetadata(!scene);
    setAllowDuplicateDownload(false);
    setError(null);
  }, [open, scene?.id, scene?.urls]);
  ...
  const matchMutation = useMutation({
    mutationFn: async () => {
      ...
      return results.filter((match) => match.supportedEntity.toLowerCase() === "scene");
    },
    onSuccess: (results) => {
      ...
      setError(results.length === 0 ? "No scene downloader matched that URL." : null);
    },
  ...
  const startDownloadMutation = useMutation({
    mutationFn: async ({ queueDownload }: { queueDownload: boolean }) => {
      ...
      if (!sceneId) {
        const createdScene = await scenes.create({
          title: deriveSceneTitle(normalizedUrl, selectedMatch.label),
          organized: false,
          urls: [normalizedUrl],
        });
      ...
      return { sceneId, queued: queueDownload };
    },
    onSuccess: ({ sceneId, queued }) => {
      if (queued) {
  ...
  const title = scene ? "Download Scene Media" : "New Scene From URL";
  const subtitle = scene
    ? `Attach a downloader result to ${scene.title || `Scene ${scene.id}`}.`
    : "Create a scene from a source URL now and choose whether to queue the media download immediately or later.";
      ...
      <div className="w-full max-w-2xl overflow-hidden rounded-2xl border border-border bg-surface shadow-2xl">
          ...
          <div className="space-y-2">
            ...
            {orderedMatches.length === 0 ? (
              <div className="rounded-xl border border-dashed border-border bg-card/50 px-4 py-5 text-sm text-muted">
                Match the URL to choose a scene downloader.
              </div>
            ) : (
              <div className="space-y-2">
              ...
              <label className="flex items-start gap-3 rounded-xl border border-border bg-card/60 px-4 py-3 text-sm text-foreground">
                ...
                <span>
                  ...
                  <span className="mt-1 block text-xs text-secondary">
                    When the downloader exposes metadata, Cove will merge it into the scene after the media import finishes.
                  </span>
                </span>
              </label>
              <label className="flex items-start gap-3 rounded-xl border border-border bg-card/60 px-4 py-3 text-sm text-foreground">
                ...
                <span>
                  ...
                  <span className="mt-1 block text-xs text-secondary">
                    Leave this off to stop Cove from creating or queueing a scene when this source URL already has downloaded files.
                  </span>
                </span>
              </label>
            </div>
        ...
        <div className="flex items-center justify-between border-t border-border px-5 py-4">
          <div className="text-xs text-muted">
            {scene
              ? "The scene stays editable while the download job runs. Downloads are queued separately from exclusive sync jobs."
              : "Cove checks for duplicate downloaded URLs before it creates a new scene or queues the download."}
          </div>
