...
interface Props {
  ...
  onAccept: (performerId: number) => void;
  onReject: (performerId: number) => void;
...
export function FaceSuggestionsPanel({
  ...
  return (
    <div className="space-y-3 border-t border-border pt-4">
      <div>
        <div className="text-xs font-semibold uppercase tracking-wide text-muted">Suggested performers</div>
        <p className="mt-1 text-sm text-secondary">Extension-provided matches ranked by confidence and supporting face evidence.</p>
      </div>

      {isLoading ? (
        <p className="text-xs text-secondary">Loading performer suggestions...</p>
      ) : suggestions.length === 0 ? (
        <p className="rounded-xl border border-dashed border-border px-3 py-4 text-sm text-secondary">No suggestions yet - run AI.Faces.</p>
      ) : (
              ...
              <div className="flex gap-4">
                ...
                <div className="min-w-0 flex-1 space-y-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <button
                      type="button"
                      onClick={() => onAccept(suggestion.performerId)}
                      disabled={disabled}
                    ...
                    <button
                      type="button"
                      onClick={() => onReject(suggestion.performerId)}
                      disabled={disabled}
