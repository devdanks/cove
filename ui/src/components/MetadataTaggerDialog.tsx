...
function MetadataTaggerShell({ open, title, subtitle, onClose, children }: { open: boolean; title: string; subtitle: string; onClose: () => void; children: ReactNode }) {
  if (!open) return null;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" role="dialog" aria-modal="true" aria-label={title}>
      <div className="flex max-h-[92vh] w-full max-w-7xl flex-col overflow-hidden rounded-xl border border-border bg-surface shadow-2xl">
        <div className="flex items-start justify-between gap-4 border-b border-border px-5 py-4">
          ...
          <button type="button" onClick={onClose} className="text-muted hover:text-foreground" aria-label="Close metadata tagger dialog">
            <X className="h-5 w-5" />
          </button>
        </div>
...
export function PerformerMetadataTaggerDialog({ open, onClose, performer, onNavigate }: { open: boolean; onClose: () => void; performer: Performer; onNavigate: (route: any) => void }) {
  return (
    <MetadataTaggerShell
      open={open}
      onClose={onClose}
    ...
    >
      <PerformerTagger performers={[performer]} onNavigate={(performerId) => onNavigate({ page: "performer", id: performerId })} mode="detail" />
    </MetadataTaggerShell>
  );
}

export function StudioMetadataTaggerDialog({ open, onClose, studio }: { open: boolean; onClose: () => void; studio: Studio }) {
  return (
    <MetadataTaggerShell
      open={open}
      onClose={onClose}
