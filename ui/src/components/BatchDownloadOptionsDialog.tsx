    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
      <div className="w-full max-w-2xl overflow-hidden rounded-2xl border border-border bg-surface shadow-2xl">
        ...
        <div className="space-y-4 px-5 py-4">
          {entity === "Scene" ? (
            <label className="flex items-start gap-3 rounded-xl border border-border bg-card px-4 py-3 text-sm text-foreground">
              <input
                type="checkbox"
                checked={!!options.scrapeScenes}
                onChange={(event) => setOptions((current) => ({ ...current, scrapeScenes: event.target.checked }))}
                className="mt-0.5 h-4 w-4 rounded border-border bg-card text-accent focus:ring-0"
              />
              <span>
                <span className="block font-medium">Scrape downloaded scenes after download</span>
                <span className="mt-1 block text-xs text-secondary">Apply the normal scene scrape pass once each downloaded scene is imported.</span>
              </span>
            </label>
          ) : null}
        ...
        <div className="flex items-center justify-end gap-2 border-t border-border px-5 py-4">
          ...
          <button
            onClick={() => onConfirm(normalizeBatchDownloadOptions(options))}
            disabled={isPending}
